-- Kiyo Food 0057: retry the live E2E cleanup with order-safe route quote
-- deletion. This supersedes any cached copy of 0056.

BEGIN;

CREATE TEMP TABLE IF NOT EXISTS kiyo_0057_cleanup_result (
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0057 safety stop: matched restaurant is not an E2E test restaurant.';
  END IF;

  IF COALESCE(array_length(v_restaurant_ids, 1), 0) > 3 THEN
    RAISE EXCEPTION '0057 safety stop: too many restaurants matched.';
  END IF;
  ...
  IF COALESCE(array_length(v_profile_ids, 1), 0) > 12 THEN
    RAISE EXCEPTION '0057 safety stop: too many test profiles matched.';
  END IF;
