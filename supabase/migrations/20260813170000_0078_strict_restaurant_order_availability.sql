-- New orders and trusted route quotes are accepted only while a published
-- restaurant is explicitly open. The existing "busy" status is a pause for
-- overload protection; historical orders and financial snapshots are untouched.
...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR v_restaurant.operational_status <> 'open'
...
COMMENT ON FUNCTION public.restaurant_accepts_orders(uuid, timestamptz) IS
  'Authoritative new-order availability: published, explicitly open, not on vacation, and within configured hours.';
