...
export function useRealtime(
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      chan = chan.on(
        'postgres_changes',
        { event: '*', schema: 'public', table, filter: filter ? stringifyFilter(filter) : undefined },
  ...
  }, [table, enabled, JSON.stringify(filter), backoffMs, backoffMaxMs]);

  return { status };
}

function stringifyFilter(filter: Record<string, string>): string | undefined {
  const keys = Object.keys(filter);
  if (keys.length === 0) return undefined;
  // PostgREST filter syntax: "restaurant_id=eq.<uuid>"
  return keys.map((k) => `${k}=${filter[k]}`).join(',');
}
