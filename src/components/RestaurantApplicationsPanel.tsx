...
const labels = {
  en: { title: 'Restaurant applications', waiting: 'Waiting on you', search: 'Search applications', empty: 'No applications match this filter.', details: 'Application file', start: 'Start review', changes: 'Request changes', reject: 'Reject', approve: 'Preliminary approval', menu: 'Start menu review', ready: 'Mark ready to publish', publish: 'Publish restaurant', suspend: 'Suspend', reactivate: 'Reactivate onboarding', reason: 'Decision reason shown to applicant', notes: 'Private admin notes', saveNotes: 'Save private notes', foodRate: 'Approved food commission (%)', deliveryRate: 'Approved delivery share (%)', base: 'Commission base', all: 'All', refresh: 'Refresh', readiness: 'Publication readiness', readyNow: 'All publication requirements are complete.', blocked: 'Publication is blocked by:' },
  fr: { title: 'Demandes de restaurants', waiting: 'En attente de vous', search: 'Rechercher une demande', empty: 'Aucune demande ne correspond à ce filtre.', details: 'Dossier de demande', start: 'Commencer l’examen', changes: 'Demander des modifications', reject: 'Rejeter', approve: 'Approbation préliminaire', menu: 'Commencer l’examen du menu', ready: 'Marquer prêt à publier', publish: 'Publier le restaurant', suspend: 'Suspendre', reactivate: 'Réactiver l’intégration', reason: 'Motif de décision visible par le demandeur', notes: 'Notes administrateur privées', saveNotes: 'Enregistrer les notes privées', foodRate: 'Commission nourriture approuvée (%)', deliveryRate: 'Part livraison approuvée (%)', base: 'Base de commission', all: 'Toutes', refresh: 'Actualiser', readiness: 'État de préparation à la publication', readyNow: 'Toutes les conditions de publication sont remplies.', blocked: 'La publication est bloquée par :' },
  ar: { title: 'طلبات المطاعم', waiting: 'بانتظار قرارك', search: 'البحث في الطلبات', empty: 'لا توجد طلبات مطابقة.', details: 'ملف الطلب', start: 'بدء المراجعة', changes: 'طلب تعديلات', reject: 'رفض', approve: 'موافقة أولية', menu: 'بدء مراجعة القائمة', ready: 'تحديد كجاهز للنشر', publish: 'نشر المطعم', suspend: 'تعليق', reactivate: 'إعادة تفعيل الإعداد', reason: 'سبب القرار الظاهر لمقدم الطلب', notes: 'ملاحظات إدارية خاصة', saveNotes: 'حفظ الملاحظات الخاصة', foodRate: 'عمولة الطعام المعتمدة (%)', deliveryRate: 'حصة التوصيل المعتمدة (%)', base: 'أساس العمولة', all: 'الكل', refresh: 'تحديث', readiness: 'جاهزية النشر', readyNow: 'اكتملت جميع متطلبات النشر.', blocked: 'النشر متوقف بسبب:' },
} as const;
...
export function RestaurantApplicationsPanel() {
  ...
  const preliminaryApprove = async () => {
    ...
    if (!Number.isFinite(food) || food < 0 || food > 1 || !Number.isFinite(delivery) || delivery < 0 || delivery > 1) {
      setError('Approved rates must be between 0% and 100%.');
      return;
    }
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {(unreadByApplication[application.id] ?? 0) > 0 && (
                      <span className="ml-1 inline-flex rounded-full bg-ember-600 px-2 py-0.5 text-[11px] font-bold text-white">
                        {unreadByApplication[application.id]} new
                      </span>
                    )}
                  </div>
                </div>
        ...
        {selected && (
          <div className="space-y-5 rounded-xl border border-ink-100 bg-white p-4 sm:p-5">
            ...
            <dl className="grid gap-3 text-sm sm:grid-cols-2">
              <Info label="Legal name" value={selected.legal_name ?? '—'} />
              <Info label="Phone" value={selected.phone} />
              <Info label="Cuisine" value={selected.cuisine.join(', ') || '—'} />
              <Info label="Address" value={selected.address} />
              <Info label="Delivery radius" value={`${selected.max_delivery_km} km`} />
              <Info label="Minimum order" value={`${selected.min_order_amount} DZD`} />
              <Info label="Coordinates" value={selected.latitude == null ? '—' : `${selected.latitude.toFixed(6)}, ${selected.longitude?.toFixed(6)}`} />
              <Info label="Proposed food commission" value={selected.proposed_food_commission_rate == null ? 'Not proposed' : `${(Number(selected.proposed_food_commission_rate) * 100).toFixed(2)}%`} />
              <Info label="Proposed delivery share" value={selected.proposed_delivery_share_rate == null ? 'Not proposed' : `${(Number(selected.proposed_delivery_share_rate) * 100).toFixed(2)}%`} />
            </dl>

            {selected.description && <div><p className="text-xs font-semibold text-ink-400">Description</p><p className="mt-1 whitespace-pre-wrap text-sm text-ink-700">{selected.description}</p></div>}
            ...
            {selected.status === 'under_review' && (
              <div className="grid gap-3 rounded-xl border border-ink-100 bg-ink-50 p-3 sm:grid-cols-2">
                ...
                <label className="sm:col-span-2"><span className="kiyo-label">{tx.base}</span><select className="kiyo-input" value={commissionBase} onChange={(event) => setCommissionBase(event.target.value as typeof commissionBase)}><option value="food_subtotal">Food subtotal only</option><option value="food_plus_delivery">Food and delivery combined</option></select></label>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {selected.status === 'published' && <Action icon={<XCircle className="h-4 w-4" />} label={tx.suspend} danger disabled={acting} onClick={() => transition('suspended', true)} />}
...
function Action({ label, icon, onClick, disabled, primary, danger }: { label: string; icon: React.ReactNode; onClick: () => void; disabled: boolean; primary?: boolean; danger?: boolean }) {
  return <button type="button" onClick={onClick} disabled={disabled} className={`min-h-11 rounded-xl px-3 py-2 text-xs font-bold transition disabled:opacity-50 ${primary ? 'bg-ink-900 text-white hover:bg-ink-800' : danger ? 'border border-error-200 text-error-700 hover:bg-error-50' : 'border border-ink-200 text-ink-700 hover:bg-ink-50'}`}>{icon}<span className="ml-1.5">{label}</span></button>;
}
